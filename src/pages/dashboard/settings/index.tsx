...
import { useRolesList, useDeleteRole, useCreateRole, CreateRoleRequest } from '../../../hooks/useRole'
import { checkPermissions } from '../../../utils/helper'
import { useAuth } from '../../../hooks/useAuth'
import { AddRoleModal, DeleteRoleModal } from '../../../components/modals'
...
import { PermissionCreateRequest } from '../../../api/permissions'

function Settings() {
  const [activeTab, setActiveTab] = useState('authentication')
  const [searchTerm, setSearchTerm] = useState('')
  const [showRoleModal, setShowRoleModal] = useState(false)
  const [deleteRoleModal, setDeleteRoleModal] = useState({ isOpen: false, roleId: '', roleName: '' })
  const { currentUser } = useAuth()
  const createPermission = useCreatePermission()
  const [showPermissions, setShowPermissions] = useState(false)


  const { data: rolesData, isLoading: rolesLoading } = useRolesList({ page: 1, limit: 100 })
  ...
  // Roles Data
  const rolesResult: any = rolesData?.result
  const roles = Array.isArray(rolesResult?.roles) ? rolesResult.roles : Array.isArray(rolesResult) ? rolesResult : []
  ...
  // operations of permission

  const handleshowCreatePermissionsModale = () => {
    setShowPermissions(true)
  }
  const handleSubmiPermissionCreated = async (payload: PermissionCreateRequest) => {
    try {
      const response = await createPermission.mutateAsync(payload)
      if (response) {
        setShowPermissions(false)
      } else {
        toast.error('Failed to create permission', { autoClose: 2000 })
      }
    } catch (error) {
  ...
  const handleAddRole = async (roleData: CreateRoleRequest) => {
    try {
      ...
      const result = await createRole.mutateAsync(roleWithPermissions)
  ...
  const handEditRole = (roleId: string) => {
    navigate(`/dashboard/settings/edit-role/${roleId}`)
  }

  const handleDeleteRole = async (roleId: string, roleName: string) => {
    setDeleteRoleModal({ isOpen: true, roleId, roleName })
  }

  const confirmDeleteRole = async () => {
    try {
      await deleteRole.mutateAsync(deleteRoleModal.roleId)
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto mt-6 scrollbar-hide" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200">
          <div className="border-b border-gray-200">
            <nav className="flex space-x-8 px-6 bg-[#1A3263]">
              {currentUser && checkPermissions(currentUser, 'role:read') && (
                <button
                  onClick={() => setActiveTab('webinfo')}
                  className={`py-4 px-1 border-b-3 font-medium  text-medium !font-bold ${activeTab === 'webinfo'
              ...
              <button
                ...
                className={`py-4 px-1 border-b-2 font-medium text-medium !font-bold ${activeTab === 'authentication'
                  ? 'border-yellow-500 text-white'
                  : 'border-transparent text-gray-300 hover:text-gray-400'
                  }`}
              >
              ...
              {currentUser && checkPermissions(currentUser, 'role:read') && (
                <button
                  ...
                  className={`py-4 px-1 border-b-4 font-medium text-medium !font-bold ${activeTab === 'roles'
                    ? 'border-yellow-500 text-white'
                    : 'border-transparent text-gray-300 hover:text-gray-400'
                    }`}
                >
              ...
              {currentUser && checkPermissions(currentUser, 'role:read') && (
                <button
                  ...
                  className={`py-4 px-1 border-b-2 font-medium text-medium !font-bold ${activeTab === 'logs'
                    ? 'border-yellow-500 text-white'
                    : 'border-transparent text-gray-300 hover:text-gray-400'
                    }`}
                >
          ...
          {/* Web Info Tab */}
          {currentUser && checkPermissions(currentUser, 'setting:update') && activeTab === 'webinfo' && (
            <div className="p-6">
          ...
          {/* Roles Tab */}
          {currentUser && checkPermissions(currentUser, 'setting:read') && activeTab === 'roles' && (
            <div className="p-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-semibold text-gray-900">Role Management</h2>
                <div className='flex gap-2' >
                  {currentUser && checkPermissions(currentUser, 'role:create') && (
                ...
                {currentUser && checkPermissions(currentUser, 'permission:create') && (
                  <button className='bg-[#1A3263] text-white px-4 py-2 rounded-lg hover:bg-blue-800 flex gap-1 items-center justify-center' onClick={handleshowCreatePermissionsModale}><FaPlus /> Create Permission</button>
                )}
                </div>
              </div>
                    ...
                    <tbody>
                      {filteredRoles.map((role: any) => (
                        <tr key={role.id} className="border-b border-gray-100 hover:bg-gray-50">
                          ...
                          <td className="py-4 px-4 text-gray-700">{role.description || 'No description'}</td>
                          <td className="py-4 px-4">
                          ...
                          <td className="py-4 px-4">
                            <div className="flex items-center gap-2">
                             {currentUser && checkPermissions(currentUser, 'role:update') && (
                                <button className="p-2 text-blue-600 hover:text-blue-800" onClick={() => handEditRole(role.id)}>
                                  <FaEdit size={14} />
                                </button>
                              )}
                              {currentUser && checkPermissions(currentUser, 'role:delete') && (
                                <button
          ...
          {/* System Logs Tab */}
          {currentUser && checkPermissions(currentUser, 'setting:read') && activeTab === 'logs' && (
            <div className="p-6">
        ...
        <DeleteRoleModal
          ...
          onClose={() => setDeleteRoleModal({ isOpen: false, roleId: '', roleName: '' })}
          onConfirm={confirmDeleteRole}
          roleName={deleteRoleModal.roleName}
        />
        <CreatePermission
          isOpen={showPermissions}
          onClose={() => setShowPermissions(false)}
          onSubmit={handleSubmiPermissionCreated}
        />
      </div>
    </div>
  )
